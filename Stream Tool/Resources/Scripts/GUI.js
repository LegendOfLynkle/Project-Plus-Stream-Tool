...
customElements.define(
  ...
  class extends HTMLElement {
    async connectedCallback(shadowRoot = this.shadowRoot || this.attachShadow({ mode: "open" })) {
...
export async function init() {
  ...
  scores.push(new Score(document.getElementById("scoreBox1")), new Score(document.getElementById("scoreBox2")));
  ...
  if (inside.electron) {
    ...
    // remote GUIs will ask about the current main GUI state
