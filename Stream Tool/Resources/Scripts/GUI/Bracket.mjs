...
bRoundSelect.addEventListener("change", () => {
  createEncounters();
});
...
async function createEncounters(sameRound) {
  ...
  if (!sameRound && bracketPlayers[0]) {
    // not same as previous round and not first run
  ...
  for (let i = 0; i < bracketData[bRoundSelect.value].length; i++) {
    ...
    if (i % 2 == 0) {
      ...
      copyFromButt.innerHTML =
        '<div class="pInfoIconCont"><load-svg src="SVGs/Arrow.svg" class="pInfoIcon"></load-svg></div>';
