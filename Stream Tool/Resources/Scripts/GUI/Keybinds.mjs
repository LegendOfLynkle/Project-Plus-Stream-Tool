...
export function loadKeybinds() {
  ...
  Mousetrap.bind(
    ...
    () => {
      ...
      } else if (profileInfo.isVisible()) {
        // if player info menu is up
        profileInfo.apply();
        profileInfo.hide();
  ...
  Mousetrap.bind("esc", () => {
    if (inside.settings || inside.bracket) {
      viewport.toCenter();
    } else if (charFinder.isVisible() || skinFinder.isVisible() || commFinder.isVisible() || playerFinder.isVisible()) {
  ...
  Mousetrap.bind("f1", () => {
    scores[0].giveWin();
    if (settings.isScoreAutoChecked()) {
      writeScoreboard();
    }
  });
  Mousetrap.bind("f2", () => {
