...
export class PlayerGame extends Player {
  ...
  async setScImg() {
    ...
    promises.push(getRecolorImage(this.shader, this.char, this.skin, this.charInfo.colorData, folder, this.randomImg));
    promises.push(this.getBrowserSrc(this.char, this.skin, folder, this.randomImg));
  ...
  getScCharPos() {
    ...
    if (charPos.scoreboard.neutral) {
      if (charPos.scoreboard[this.skin.name]) {
        ...
        // if none of the above, use a default position
      ...
      // if there are no character positions, set positions for "Random"
      if (this.pNum % 2 == 0) {
        scCharPos[0] = 30;
      } else {
        scCharPos[0] = 30;
  ...
  getVsCharPos() {
    ...
    if (charPos.vsScreen) {
      if (charPos.vsScreen[this.vsSkin.name]) {
        // if the skin has a specific position
  ...
  resizeInput() {
    this.nameInp.style.width =
      this.getTextWidth(
        this.nameInp.value,
        window.getComputedStyle(this.nameInp).fontSize + " " + window.getComputedStyle(this.nameInp).fontFamily,
      ) +
      12 +
      "px";
  }
