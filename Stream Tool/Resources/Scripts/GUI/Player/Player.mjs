...
export class Player {
  ...
  setFinderListeners() {
    ...
    this.nameInp.addEventListener("focusout", () => {
      if (!inside.finder) {
        //but not if the mouse is hovering a finder
        playerFinder.hide();
      }
    });
  ...
  async setIconImg() {
    const promises = [];
    promises.push(getRecolorImage(this.shader, this.char, this.skin, this.charInfo.colorData, "Icons", "Icon"));
  ...
  async loadSkinImages() {
    if (!this.#skinsLoaded) {
      // only first time skin finder is opened
      ...
      for (let i = 0; i < this.skinImgs.length; i++) {
        ...
        if (currentChar == this.char) {
          ...
          finalImg.decode().then(() => {
            ...
            skinFinder.positionCharImg(this.charInfo.skinList[i].name, finalImg, { gui: this.charInfo.gui });
            // attach it
