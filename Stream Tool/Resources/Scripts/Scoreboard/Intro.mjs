...
class ScoreboardIntro {
  ...
  play() {
    ...
    if (data.score[0] + data.score[1] == 0) {
      ...
      for (let i = 0; i < maxSides; i++) {
        ...
        if (data.gamemode == 1) {
          ...
          if (data.teamName[i] == data.color[i].name + " Team") {
            pIntroEls[i].textContent = data.player[i].name + " & " + data.player[i + 2].name;
          } else {
            // else, show the team name
      ...
      if (data.score[0] + data.score[1] != 4) {
        ...
        if (data.round.toUpperCase() == "TRUE FINALS") {
          ...
          if (data.round.toLocaleUpperCase() == "GRAND FINALS" && !(data.wl[0] == "L" && data.wl[1] == "L")) {
            fadeIn(superCoolQuestionMark, 1.5, introDelaySc + 0.5);
          }
        }
      }
    }
    ...
    fadeIn(textsIntro, fadeInTimeSc, introDelaySc - 0.2);

    // aaaaand fade out everything
    fadeOut(introDiv, fadeInTimeSc + 0.2, introDelaySc + 1.8);
  }
