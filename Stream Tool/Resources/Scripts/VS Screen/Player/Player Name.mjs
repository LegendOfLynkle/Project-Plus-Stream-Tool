...
export class PlayerName {
  ...
  async update(name, tag, state) {
    ...
    let delayTime = introDelayVs + 0.3;
    ...
    if (this.getName() || this.getTag() || this.getState()) {
      // only if theres content
  ...
  show() {
    fadeIn(this.#wrapperEl, fadeInTimeVs, introDelayVs + 0.3);
    fadeIn(this.#stateImg, fadeInTimeVs, introDelayVs + 0.3);
